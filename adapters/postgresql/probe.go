...
SELECT row_to_json(clusterguard_probe) FROM (
  SELECT
    ...
    COALESCE((SELECT latest_end_lsn::text FROM receiver), '') AS receiver_latest_end_lsn,
    CASE
      WHEN pg_is_in_recovery()
        ...
        AND (
          ...
          OR (
            (SELECT status FROM receiver) = 'streaming'
            AND (SELECT latest_end_lsn FROM receiver) = pg_last_wal_replay_lsn()
          )
        )
      THEN 0
    ...
    CASE
      WHEN pg_is_in_recovery()
      THEN COALESCE(
        (SELECT received_tli::text FROM receiver),
...
func discover(ctx context.Context, runner SQLRunner, request adapter.DiscoverRequest) (adapter.DiscoveryResult, error) {
	...
	// The registered endpoint is authoritative. In a container runtime the
	// server reports its container port (for example 5432), while clients and
	// peers must keep using the published host port (for example 55432).
	...
	instance := model.DatabaseInstance{
		...
		EngineMetadata: map[string]string{
			...
			"receiver_latest_end_lsn": probe.receiverLatestEndLSN,
		},
	}
...
func parseIdentityProbe(row Row) (identityProbe, error) {
	...
	receiverLatestEndLSN := strings.TrimSpace(row["receiver_latest_end_lsn"])
	for name, value := range map[string]string{
		"current_lsn": currentLSN, "receive_lsn": receiveLSN, "replay_lsn": replayLSN,
		"receiver_latest_end_lsn": receiverLatestEndLSN,
	} {
		if value != "" {
	...
	return identityProbe{
		...
		replayLSN:            replayLSN,
		receiverLatestEndLSN: receiverLatestEndLSN,
