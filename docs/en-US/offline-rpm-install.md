...
The default MySQL HA deployment with VIP enables automatic database-level failover, not relying on VMware virtual machine names or VMX paths. The control plane detects the MySQL port and SQL health every 5 seconds; after the primary database fails six times consecutively and covers 30 seconds, the Raft Leader can issue a candidate primary database transition lease. The old primary Agent caches the majority authorization for a maximum of 10 seconds and checks it locally every 5 seconds; after losing authorization, it will revoke the VIP, record the persistent isolation intent, and keep MySQL read-only. The control plane waits for a 15-second isolation window and confirms the Raft majority, fault evidence, and precise transition lease again before allowing the candidate database to promote.
