...
Each host also runs one `clusterguard-agent`. The `clusters` array in `/etc/clusterguard/agent.json` may contain host PostgreSQL, host MySQL, and Docker MySQL policies together. Distinct `cluster_id`, `instance_id`, and `runtime_kind` values identify them; equal database ports do not imply equal clusters or runtimes.
