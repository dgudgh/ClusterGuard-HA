...
A new metadata store bootstraps `admin` with role `admin` and
`MustChangePassword=true`. The first-login password is the value of
`bootstrap_admin_password_env` when the deployment sets it, and is otherwise
generated by the Leader into a root-only `bootstrap-admin-password` file (mode
0600) beside the metadata file. Existing users are
