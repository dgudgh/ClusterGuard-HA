...
> **版本谱系**：`2.2-103` 有两个本地构建。**首版**（`2026-09-22T03:25:36Z`，基线
> `55de30f`）构建于许可落地之前，包内既无许可文本、RPM 头也写着 `Proprietary`。
> **本目录交付的是重建的第二版**（基线 `68c3448`），唯一目的是把 `AGPL-3.0-only`
> 的声明补进产物本身。首版从未发布、从未交付，其产物改名保留在 `.build/` 下备查。

## 本次重建（第二版）改了什么

2.2-103 首版构建时，仓库还没有任何许可声明：根目录没有 `LICENSE`，
`packaging/rpm/nfpm.yaml` 写着 `license: Proprietary`。许可随后落地为
`AGPL-3.0-only`（提交 `68c3448`），但**已产出的介质不会因此改变**——RPM 头的
`License` 字段是打包那一刻写死的，介质内的文件是构建时复制的。因此带许可重新构建：

| 项 | 首版 | 本版（重建） |
| --- | --- | --- |
| RPM 头 `License` | `Proprietary` | `AGPL-3.0-only` |
| RPM 内 `/usr/share/doc/clusterguard-ha/` | 无许可文件 | `LICENSE`、`THIRD-PARTY-NOTICES.md`、`MPL-2.0.txt` |
| 介质根目录 | 无许可文件 | `LICENSE`、`THIRD-PARTY-NOTICES.md`、`MPL-2.0.txt` |

**为什么必须重出而不是只改声明**：`AGPL-3.0` §4 要求分发目标代码时随附本许可的副本；
按首版现状发给客户属于不合规分发。除许可外，代码、包内文档、依赖闭包与数据库包与首版
一致——本版**不含**任何功能或行为变更，因此**装维流程与首版完全相同**。

## 相对 2.2-102 的变更

2.2-102 的代码基线是 `7b36461`（包内 `RELEASE-INFO` 记录打包时 HEAD `2b9a449`，该提交
只新增发布说明与索引、不含代码改动，两者代码等价）。2.2-103 **首版**基线为 `eb741ee`；
**本重建版**基线为 `68c3448`，相对首版只多出许可落地相关改动（见上一节）。
...
8. 本版**介质**未上传 GitHub。仓库自身的 `LICENSE`（`AGPL-3.0-only`，提交 `68c3448`）
   已随文档分支发布，公开仓库页已识别该许可；但这不等于介质已发布。
