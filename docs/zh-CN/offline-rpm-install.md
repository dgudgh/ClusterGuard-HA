...
带 VIP 的 MySQL HA 部署默认启用数据库层自动故障切换，不依赖 VMware 虚拟机名称或 VMX 路径。控制面每 5 秒探测 MySQL 端口和 SQL 健康；主库连续 6 次失败并覆盖 30 秒后，Raft Leader 才能发布候选主库过渡租约。旧主 Agent 最多只缓存 10 秒多数派授权，并每 5 秒本地核对一次；失去授权后会撤销 VIP、记录持久隔离意图，并把 MySQL 保持为只读。控制面等待 15 秒隔离窗口并再次确认 Raft 多数、故障证据和精确过渡租约后，才允许候选库提升。
