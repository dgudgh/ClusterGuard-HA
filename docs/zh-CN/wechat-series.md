...
这一组文章从 ClusterGuard HA 的来路讲起，随后分别讨论 MHA 和 Orchestrator，再进入产品机制、离线安装、日常操作和生产验收。七篇文章可以独立阅读，连起来则是一份从产品认识到落地运行的完整说明。
...
ClusterGuard HA `2.1.45` 是 2.1 系列封板版本，正式支持范围是 MySQL 高可用。PostgreSQL 从 2.2 系列开始开发和验收。Oracle Data Guard Broker 与 SQL Server Always On 属于后续独立适配和认证范围，不能用 MySQL 的测试结论替代。

系列中的产品能力、命令和截图均以仓库文档及 2.1.45 交付物为依据。MHA 与 Orchestrator 的描述引用各自官方项目资料。部署到生产前，仍需按现场操作系统、数据库版本、网络、存储和隔离条件重新验收。
