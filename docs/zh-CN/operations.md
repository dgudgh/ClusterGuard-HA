...
恢复控制器仅在多数 Leader 上执行。发现连续记录 4 次当前主库失败，且观测跨度不少于 3 秒后，才生成一个事件。控制器仅选择排名第一的合格候选者，并通过私有内部授权路径提交一个正常的持久 `failover` 操作。公共 JSON 无法选择此模式。事件 ID 在 `APPROVE` 上进行审计；安全防护、锁定、隔离、执行、验证、审计和报告仍然是强制性的。事件派生的幂等性密钥防止 Leader 变更后重复成功或不确定的故障转移。被阻止的尝试至少等待配置的重试周期。
