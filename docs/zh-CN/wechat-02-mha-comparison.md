...
| 关注点 | MHA | ClusterGuard HA |
| --- | --- | --- |
| 主要目标 | MySQL 主库故障切换与在线切换 | 数据库高可用控制、端点协调、恢复和交付 |
| 运行结构 | Manager 通过 SSH 调用 Node 工具 | 三个或更多奇数控制节点、Raft、受限 Agent |
| 节点身份 | 以主机和实例配置为主要入口 | 平台 `resource_id` 与数据库原生身份 |
| 候选选择 | 依据复制位置、配置和候选规则 | Adapter 依据清单、健康、GTID 或 WAL、延迟和门禁评估 |
| 业务入口 | 通过自定义脚本接入 VIP 等方案 | 主库角色与 HA Endpoint 进入同一操作计划 |
| 并发控制 | 依赖 Manager 运行和现场约束 | 集群级持久操作锁与 Leader 多数派门禁 |
| 旧主恢复 | 由后续脚本或运行手册处理 | MySQL 增量或重建，PostgreSQL `pg_rewind` 或 `pg_basebackup` |
| 审计交付 | 日志和外部系统集成 | 操作、验证、审计、报告和中文控制台 |
| 安装方式 | 按项目组件和现场依赖部署 | 带校验的 RPM 与离线安装包 |
| 数据库边界 | MySQL | 2.1 稳定支持 MySQL，2.2 已通过 PostgreSQL 16.4 三节点实验室验收 |
