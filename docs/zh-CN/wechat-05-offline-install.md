...
2.1.45 的生产支持范围是 MySQL。2.2 候选版本线已经通过 PostgreSQL 16.4 三节点实验室验收，但本文仍是 2.1.45 MySQL 离线安装教程，不能直接替换引擎名称后用于 PostgreSQL 生产部署。PostgreSQL 安装应以 2.2 对应交付物和 [PostgreSQL 高可用操作说明](postgresql-ha.md) 为准。
...
初始账号是 `admin`。首次密码写在控制节点上与元数据同目录的 root 专属文件 `bootstrap-admin-password`（权限 0600）里；部署若设置了 `bootstrap_admin_password_env`，则用该变量指定的口令，不再生成文件。首次登录必须立即改密。改密前，平台只允许认证和修改密码，不能读取或操作集群数据。
