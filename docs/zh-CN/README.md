...
[2.2-104 发布说明](release-2.2.104.md)：最新安装介质，代码基线 `259b796`。相对 2.2-103 新增**运行参数只读视图**（控制台「设置 → 运行参数」展示配置文件的实际生效值、来源与「需重启」标记，凭据只显示环境变量名）与**引擎级集群策略**（自动切换观测次数、证据窗口、切换操作预算、维护抑制写入 Raft 复制存储，控制台可改、可审计、运行时每轮读取即刻生效）。策略变更与审计事件共用同一次 Raft 提交；策略读取失败或输入越界时设置页禁止写入。介质同时内嵌 MySQL 8.0.44 与 PostgreSQL 16.4 原厂介质。本地已按发版门禁构建并核验；**介质未上传 GitHub、未被现场验收**；因缺少与现场信任公钥匹配的生产补丁签名私钥，未生成 `.cgupgrade`。
...
最新安装介质先看 [2.2-104 发布说明](release-2.2.104.md)。英文侧发布说明只维护到
`release-2.2.47.md`；此后的发布说明**没有英文对应版本**，只维护简体中文，现存
`release-2.2.69.md` 至 `release-2.2.104.md`，按版本号倒序存放在本目录。
