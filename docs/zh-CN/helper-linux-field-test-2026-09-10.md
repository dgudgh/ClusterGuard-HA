...
后续状态（2026-09-11）：本文记录 9 月 10 日源码与隔离夹具的结果，保留当时的服务、版本及 UID 证据。后续 shell 私有执行区修复已纳入 [2.2-101 安装包](release-2.2.101.md)，但最终版安装后 socket、滚动升级和回退未重新验收。9 月 11 日记录的账号/unit 不可用与本文不同，当前现场状态不能仅从任一历史快照推定；后续验证边界见 [私有执行区安全记录](updater-private-workspace-2026-09-11.md)。
