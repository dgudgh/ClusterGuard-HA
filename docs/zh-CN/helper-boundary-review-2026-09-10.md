...
截至 2026-09-11 的后续状态：9 月 10 日 Linux root/服务 UID 与隔离 MySQL/PG 测试见 [Helper Linux 三节点测试](helper-linux-field-test-2026-09-10.md)；本文第 1–3 项剩余 shell 边界已在 `f76b2f4` 实施私有执行区修复，见 [升级器私有执行区记录](updater-private-workspace-2026-09-11.md)。包含修复的 [2.2-101 安装包](release-2.2.101.md) 已预发布，生产签名升级包和最终版现场升级验收仍未完成。

以下保留 9 月 10 日的原始范围、测试和结论，其中“未修复/未打包/未提交”均指当轮。历史隔离测试不能直接替代 9 月 11 日最终源码验收；两轮现场账号/unit 检查记录不一致的问题见后续安全记录。
