...
for ((index=0; index<target_count; index++)); do
  ...
  validate_remote_identity "${host}" "${user}"
  remote_exec "${host}" "${user}" "test \"\$(id -u)\" -eq 0 && command -v bash >/dev/null && command -v jq >/dev/null"
  ...
  if [[ "${kind}" == "data" || "${kind}" == "mixed" ]]; then
    ...
    remote_stdin "${payload}" "${host}" "${user}" "chmod 0700 /var/lib/clusterguard/stage/clusterguard-mysql-install.sh && /var/lib/clusterguard/stage/clusterguard-mysql-install.sh" >/dev/null
    ...
    identity="$(remote_stdin "${payload}" "${host}" "${user}" "chmod 0700 /var/lib/clusterguard/stage/clusterguard-mysql-sync.sh && /var/lib/clusterguard/stage/clusterguard-mysql-sync.sh '${method}'")"
